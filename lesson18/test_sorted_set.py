...
class TestContainerProtocol(unittest.TestCase):
    ...
    def tes_positive_not_contained(self):
...
class TestIterableProtocol(unittest.TestCase):
    ...
    def test_for_loop(self):
        ...
        expected=[1,2,7,9]
        for item in self.s:
            self.assertTrue(item,expected[index])
            index+=1
class TestSequenceProtocol(unittest.TestCase):
    ...
    def test_index_before_the_begaining(self):
        with self.assertRaises(IndexError):
            self.s[-6]
...
class TestInequalityProtocol(unittest.TestCase): #Python will implement inequality by negating the equality operator⁷
