...
def write_grayscale(filename, pixels):
    ...
    with open(filename,"wb") as bmp:
        #BMP Header
        bmp.write(b"BM")
        #The next four bytes hold the filesize as 32-bit 
        #litte endian integer.Zero placeholder for now
        ...
        for row in reversed(pixels): # BMP files are bottom to top
            row_data = bytes(row)
            bmp.write(row_data)
            padding = b'\x00' * ((4 - (len(row) % 4)) % 4) # Pad row to multiple
            # of four bytes
            bmp.write(padding)
